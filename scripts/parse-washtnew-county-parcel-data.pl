...
my $SURVEY_URL = 'https://survey.washtenawbroadband.org';
...
my $merged_headers =
  [grep {!/^jurisdicti$/}
      uniq('jurisdiction', 'participant_id', 'participant_url', 'parcel_type', @{$res_headers}, @{$comm_headers})];
...
my $parcel_cache_dir = File::Spec->catfile($ENV{HOME}, '.local', 'var', 'piecewise');
...
for my $line (@lines) {
    ...
    $new_line->{participant_url} = qq($SURVEY_URL/$new_line->{participant_id});
